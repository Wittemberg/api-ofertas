...
Pastas: .github/workflows/ | lib/ | prisma/ | routes/
Rotas: auth.js | products.js | stores.js | categories.js | offers.js
Raiz: server.js | prisma.js | package.json | Dockerfile | docker-compose.yml

## 🗄️ Modelo

**tenants** id (UUID), name, slug, domain, is_active, timestamps
**users** id (UUID), tenant_id (FK), name, email (unique/tenant), password_hash, role, is_active
**stores** id (UUID), tenant_id (FK), name, slug (unique/tenant), city, state, address, phone, is_active
**categories** id (UUID), tenant_id (FK), name, slug (unique/tenant), is_active
**products** id (UUID), tenant_id (FK), category_id (FK), internal_code (unique/tenant), barcode (unique/tenant), name, description, image_url, unit, is_active
**offers** id (UUID), tenant_id (FK), store_id (FK), product_id (FK), price_from, price_to, starts_at, ends_at, is_featured, is_active
**csv_imports** id (UUID), tenant_id (FK), file_url, status, total_rows, valid_rows, invalid_rows, warnings (JSON), errors (JSON)
**media_assets** id (UUID), tenant_id (FK), product_id (FK), file_url, file_type

## 🔐 Autenticação

JWT com authenticate e authorize('admin'). Multi-tenant por header Host.
...
| GET | /offers | JWT | Listar (page, limit, filters) |
