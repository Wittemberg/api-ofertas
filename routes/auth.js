...
const authPlugin = async function (fastify, opts) {
  ...
  fastify.post('/register', async function (request, reply) {
    const { email, password, name, domain } = request.body;
    ...
    if (request.tenant?.id) {
      ...
      return reply.code(400).send({ error: 'Informe o domínio do tenant (ex: teste.awecloudsolution.com)' });
    }
    ...
    if (existingUser) {
      return reply.code(400).send({ error: 'Email já cadastrado para este tenant' });
    }

    const hashedPassword = await bcrypt.hash(password, 10);
    const user = await prisma.users.create({
      data: { email, name, password_hash: hashedPassword, role: 'admin', tenant_id: tenantId },
  ...
  fastify.delete('/api-keys/:id', {
    ...
    const key = await prisma.api_keys.findFirst({
      where: { id: request.params.id, tenant_id: request.user.tenant_id }
    });
